...
object PolymorphicFibers extends IOApp.Simple {
  ...
  // Spawn - generalising fibers - create fibers for any effect
  trait MyGenSpawn[F[_], E] extends MonadCancel[F, E] {
    ...
    def cede: F[Unit] // a "yield" effect -- switch the thread of which the effect is being evaluated on

    def racePair[A, B](fa: F[A], fb: F[B]): F[Either[ // fundamental racing
      (Outcome[F, E, A], Fiber[F, E, B]), (Fiber[F, E, A], Outcome[F, E, B])
    ]]
  }

  trait MySpawn[F[_]] extends MyGenSpawn[F, Throwable]

  // capabilities form extending MonadCancel: pure, map/flatMap, raiseError, uncancelable, start
  ...
  //These are so the for comprehension works
  ...
  // -------------------------- original implementation start -- specific to the IO effect type --------------------------
